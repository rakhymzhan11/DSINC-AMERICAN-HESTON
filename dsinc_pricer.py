"""
dsinc_pricer.py -- damped-sinc (DSINC) American-put pricer under the Heston model.

Prices European and American puts under the Heston stochastic-volatility model with
constant coefficients.  The early-exercise premium is evaluated in a damped-sinc
(DSINC) basis from an affine tilted-CIR transform, using a piecewise-linear
representation of the exercise boundary x*(v) = a_m + b_m v over M = Nv-1 variance
segments; no conditional characteristic-function inversion is required.  The same
method also prices call options (e.g. through put-call duality).
...
class DSINCPut:
    """DSINC American-put solver (piecewise-linear boundary) under the constant-coefficient Heston model."""
