...
Each case is priced with the spectral (DSINC) method and the finite-difference
(MCS-ADI) method, so the two independent solvers also cross-check each other.
...
def price_both(S, K, T, rd, rf, theta, vov, rho, v0, kappa):
    """Return (spectral PE, PA, EEP), (FD PE, PA), and the two timings."""
    t = time.time()
...
print(f"   {'spectral':10}{sPE:>11.4f}{sPA:>11.4f}{sEEP:>10.4f}{st:>8.2f}")
...
for v0 in (0.0625, 0.25):
    print(f"   v0 = {v0}")
    print(f"   {'S':>4}{'spectral':>11}{'FD-ADI':>11}{'reference':>11}"
          f"{'err(sp)':>10}{'err(FD)':>10}")
    es, ef = [], []
